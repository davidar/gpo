...
"""Runs an entire experiment for benchmarking GPGO-EI-SA on a testbed.
...
import time
import numpy as np
from bbob import fgeneric, bbobbenchmarks

from gpgo_ei import *

argv = sys.argv[1:] # shortcut for input arguments

datapath = 'GPGO-EI-CG' if len(argv) < 1 else argv[0]
#datapath = 'GPGO-EI-SA-Matern' if len(argv) < 1 else argv[0]
...
opts = dict(algid=datapath,
            comments='Gaussian Process Global Optimisation maximising ' +
                     'Expected Improvement with Simulated Annealing')
...
def run_optimizer(fun, dim, maxfunevals, ftarget=-np.Inf):
    ...
    GPGO_EI_SA(fun, x_start, maxfunevals, ftarget)
